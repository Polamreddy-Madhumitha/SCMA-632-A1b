...
# Set file path
file_path <- "C:\\Users\\aravi\\OneDrive\\Desktop\\VCU\\Boot camp class\\Assignment 2\\IPL_ball_by_ball_updated till 2024.csv"
salary_file_path <- "C:\\Users\\aravi\\OneDrive\\Desktop\\VCU\\Boot camp class\\Assignment 2\\IPL SALARIES 2024.xlsx"
...
# Filter the IPL data for "Abhishek Sharma"
Abhishek_Sharma_data <- ipl_data %>%
  filter(Striker == "Abhishek Sharma" | Bowler == "Abhishek Sharma")


Abhishek_Sharma_data$Striker <- gsub("Abhishek Sharma", "Abhishek Sharma", Abhishek_Sharma_data$Striker)
Abhishek_Sharma_data$Bowler <- gsub("Abhishek Sharma", "Abhishek Sharma", Abhishek_Sharma_data$Bowler)

# Merge the filtered and renamed IPL data with the salary data
merged_data <- left_join(Abhishek_Sharma_data, salary_data, by = c("Striker" = "Player"))


# Calculate total runs scored by "Abhishek Sharma" in 2023
total_runs_2023 <- sum(merged_data %>% filter(Season == "2023" & Striker == "Abhishek Sharma") %>% pull(runs_scored), na.rm = TRUE)

# Calculate total wickets taken by "Krunal Pandya" in 2023
total_wickets_2023 <- sum(merged_data %>% filter(Season == "2023" & Bowler == "Abhishek Sharma") %>% pull(wicket_confirmation), na.rm = TRUE)

# Add a new column for performance in 2023 in the salary data
salary_data <- salary_data %>%
  mutate(Performance_2023_Runs = ifelse(Player == "Abhishek Sharma", total_runs_2023, NA),
         Performance_2023_Wickets = ifelse(Player == "Abhishek Sharma", total_wickets_2023, NA))
...
sample_data$Salary_2024 <- salary_data %>% filter(Player == "Abhishek Sharma") %>% pull(Salary)

# Calculate total runs and wickets in 2024
total_runs_2024 <- sum(sample_data %>% filter(Striker == "Abhishek Sharma") %>% pull(runs_scored), na.rm = TRUE)
total_wickets_2024 <- sum(sample_data %>% filter(Bowler == "Abhishek Sharma") %>% pull(wicket_confirmation), na.rm = TRUE)


# Print total runs and wickets for 2024 and the salary for 2024
cat(paste("Total Runs Scored by Abhishek Sharma in 2024:", total_runs_2024, "\n"))
cat(paste("Total Wickets Taken by Abhishek Sharma in 2024:", total_wickets_2024, "\n"))
cat(paste("Salary Paid to Abhishek Sharma in 2024:", sample_data$Salary_2024[1], "\n"))
...
performance_data <- data.frame(
  ...
  Salary = c(NA, sample_data$Salary_2024[1])
)
...
barplot(performance_data$Total_Runs, names.arg = performance_data$Year, col = "blue", main = "Performance of Abhishek Sharma (Total Runs in 2023 and 2024)", xlab = "Year", ylab = "Total Runs Scored")
barplot(performance_data$Total_Wickets, names.arg = performance_data$Year, col = "green", main = "Performance of Abhishek Sharma (Total Wickets in 2023 and 2024)", xlab = "Year", ylab = "Total Wickets Taken")
